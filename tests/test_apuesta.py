...
class TestApuesta(TestCase):
    ...
    def test_editar_apuesta(self):
        ...
        id_competidor = \
            [x for x in respuesta_al_crear_carrera["competidores"]
                if x["nombre_competidor"] == "Damian Corral"][0]["id"]
        ...
        respuesta_al_editar_apuesta = json.loads(
            solicitud_editar_apuesta.get_data())
    ...
    def test_obtener_apuesta_por_id(self):
        ...
        id_competidor = \
            [x for x in respuesta_al_crear_carrera["competidores"]
                if x["nombre_competidor"] == "Paz Manrique"][0]["id"]
        ...
        apuesta_obtenida = json.loads(
            solicitud_consultar_apuesta_por_id.get_data())
    ...
    def test_obtener_apuestas(self):
        ...
        id_competidor = \
            [x for x in respuesta_al_crear_carrera["competidores"]
                if x["nombre_competidor"] == "Zakaria Vila"][0]["id"]
        ...
        total_apuestas_antes = len(json.loads(
            solicitud_consulta_apuestas_antes.get_data()))
        ...
        solicitud_consulta_apuestas_despues = self.client.get(
            endpoint_apuestas, headers=headers)
        total_apuestas_despues = len(json.loads(
            solicitud_consulta_apuestas_despues.get_data()))
    ...
    def test_eliminar_apuesta(self):
        ...
        id_competidor = \
            [x for x in respuesta_al_crear_carrera["competidores"]
                if x["nombre_competidor"] == "Eduardo Tejera"][0]["id"]
        ...
        respuesta_al_crear_apuesta = json.loads(
            solicitud_nueva_apuesta1.get_data())
        id_apuesta = respuesta_al_crear_apuesta["id"]
        solicitud_consulta_apuestas_antes = self.client.get(
            endpoint_apuestas, headers=headers)
        total_apuestas_antes = len(json.loads(
